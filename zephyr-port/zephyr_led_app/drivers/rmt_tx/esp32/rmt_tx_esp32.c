...
static int rmt_tx_esp32_init(const struct device *dev)
{
    ...
    rmt_ll_set_group_clock_src(data->hal.regs, 0, RMT_CLK_SRC_PLL_F80M, 1, 1, 0);
#elif defined(CONFIG_SOC_ESP32S3)
    rmt_ll_set_group_clock_src(data->hal.regs, 0, RMT_CLK_SRC_APB, 1, 1, 0);
#else
    // Default fallback
    rmt_ll_set_group_clock_src(data->hal.regs, 0, 1, 1, 1, 0);
#endif
